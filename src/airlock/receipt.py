...
def verify_signature(receipt: dict, key: bytes) -> bool:
    if receipt.get("alg") != "HMAC-SHA256":
        return False
...
def verify_offline(repo: Path, receipt_path: Path, key_path: Path) -> dict:
    receipt = json.loads(receipt_path.read_text())
    key = key_path.read_bytes()
    checks = []

    checks.append({"check": "signature", "ok": verify_signature(receipt, key)})
    payload = receipt.get("payload", {})
    ...
    admitted = payload.get("decision") == "ADMITTED"
    all_exit_green = all(row.get("exit_code") == 0 and not row.get("timed_out", False) for row in command_records)
    checks.append({"check": "admission_commands_green", "ok": (not admitted) or all_exit_green})

    ok = all(row["ok"] for row in checks)
    return {"valid": ok, "checks": checks, "receipt_sha256": sha256_file(receipt_path)}
