...
from .receipt import ensure_key, verify_offline
from .runner import run_tournament
from .util import write_json


def _update_local_exclude(repo: Path) -> None:
    # Keep generated receipts/keys out of git status without modifying the repo's tracked .gitignore.
    git_dir = repo / ".git"
...
def command_init(args: argparse.Namespace) -> int:
    ...
    ensure_key(repo / ".airlock" / "receipt.key")
    ...
    if not rows:
        print("\nAirlock cannot admit unattended patches until the repo supplies at least one verification command.")
        return 2
...
def command_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    return 0 if report.get("status") == "ADMITTED" else 3
...
def command_verify(args: argparse.Namespace) -> int:
    ...
    if not key_path.exists():
        print("ERROR: local receipt key missing", file=sys.stderr)
        return 2
    try:
        result = verify_offline(repo, receipt_path, key_path)
    except Exception as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2
    print("AIRLOCK PROOF: " + ("VALID" if result["valid"] else "INVALID"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init = sub.add_parser("init", help="Discover repo checks, protect evaluator surfaces, and freeze a green baseline.")
    init.add_argument("--repo", default=".")
    init.add_argument("--timeout", type=int, default=1200)
    init.set_defaults(func=command_init)

    runp = sub.add_parser("run", help="Fan out untrusted agent candidates and eliminate everything that fails the repo boundary.")
    ...
    runp.add_argument("--no-pr", action="store_true", help="Keep the admitted branch local instead of attempting a GitHub PR.")
    runp.set_defaults(func=command_run)

    verify = sub.add_parser("verify", help="Offline deterministic validation of an Airlock proof receipt.")
