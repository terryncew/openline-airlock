"""PAYBACK-003 runner: one bounded improvement investment vs a frozen horizon.

Apparatus-corrected repeat of PAYBACK-002 (closed INCOMPLETE after
infrastructure failure mid-operating-phase). Same frozen scientific question
and design; PAYBACK-003 independently earns its own acquisition, candidate,
promotion result, acquisition debt D, operating curve, and terminal verdict.
Nothing is imported from PAYBACK-002's run.

Apparatus repairs vs PAYBACK-002 (earned by PAYBACK-002, frozen here before
any PAYBACK-003 scientific contact; see PREREGISTRATION.md):
  REPAIR 1: accounting schema — the real ledger records unresolved
     reservations under "reservation", not "retained"; payback_acct accepts
     either key. Frozen formulas unchanged.
  REPAIR 2: external launch/supervision layer (scripts/supervise.py) —
     durable stdout/stderr capture from process start, PID/start metadata,
     exit code/signal, heartbeat, and an infrastructure terminal record when
     the runner exits unexpectedly without RUN_STATUS.json. The supervisor
     classifies the termination; it never invents scientific observations.
     Run the paid path under the supervisor, not bare.
  REPAIR 3: deterministic transport circuit breaker — F.INFRA_BREAKER_N
     consecutive transport failures halt the study as
     infrastructure-INCOMPLETE (InfraCircuitBreaker) instead of permitting
     indefinite continuation into ambiguous calls.
...
Two modes:
  ...
  python payback_run.py --authorize-paid-contact --preflight
      ...
      Launch via: python supervise.py --run-dir <dir> -- python
      payback_run.py --authorize-paid-contact --preflight
...
sys.path.insert(0, "/home/hatch/workspace/openline-airlock/study/payback-003")
sys.path.insert(0, "/home/hatch/workspace/openline-airlock/study/payback-003/scripts")
...
STUDY = "/home/hatch/workspace/openline-airlock/study/payback-003"
...
class InfraCircuitBreaker(Exception):
    """Raised when the transport circuit breaker trips. paid() converts it
    to a halted RUN_STATUS: the study ends INCOMPLETE (infrastructure),
    never rescued, never continued."""


class TransportBreaker:
    """Provider wrapper: deterministic infrastructure halt rule.

    Apparatus repair (PAYBACK-003, earned by PAYBACK-002): consecutive
    transport failures indicating provider/egress instability must halt the
    study as infrastructure-INCOMPLETE instead of permitting indefinite
    continuation into hundreds of ambiguous calls.

    Counts consecutive provider posts that fail before any usable HTTP
    response exists (OSError: timeouts, connection refused/reset;
    http.client.HTTPException: remote-closed and protocol errors). Any
    completed post resets the streak. HTTP error statuses are NOT counted:
    the provider responded, and the existing unresolved semantics already
    handle them. Credential failures are NOT counted: they propagate
    untouched and halt via the existing InfrastructureHalt path.

    After F.INFRA_BREAKER_N consecutive transport failures the breaker is
    tripped (one-way; a later success does not un-trip). The runner checks
    the flag between attempts and raises InfraCircuitBreaker. Unresolved
    reservations stay retained; nothing is converted into a zero-cost
    scientific observation, and no scientific observation is invented.
    """
    ...
    def post(self, body: dict, headers: dict, timeout: float):
        try:
            out = self._inner.post(body, headers, timeout)
        except (OSError, http.client.HTTPException):
            self.streak += 1
            if self.streak >= self._n:
                self.tripped = True
            raise
        self.streak = 0
        return out
...
def _attempt(task, method_text, invocation_id, cx):
    check_stop(cx["stop"])
    breaker = cx.get("breaker")
    if breaker is not None and breaker.tripped:
        raise InfraCircuitBreaker(
            f"transport circuit breaker tripped: {breaker.streak} consecutive "
            f"transport failures; halting as infrastructure-INCOMPLETE")
...
def acquisition(tasks, alloc, cx):
    ...
    res = worker.invoke(instructions=PROPOSAL_PROMPT,
                        ...
                        stop_file=cx["stop"], raw_dir=cx["raw"])
    # Circuit-breaker check BEFORE interpreting the proposal result: a trip
    # means infrastructure instability, which must halt INCOMPLETE even if
    # this call happened to succeed (the trip is one-way).
    if cx.get("breaker") is not None and cx["breaker"].tripped:
        raise InfraCircuitBreaker(
            f"transport circuit breaker tripped: {cx['breaker'].streak} "
            f"consecutive transport failures; halting as "
            f"infrastructure-INCOMPLETE")
...
def paid(args):
    ...
    # Apparatus repair (PAYBACK-003): deterministic transport circuit
    # breaker. F.INFRA_BREAKER_N consecutive transport failures halt the
    # study as infrastructure-INCOMPLETE instead of permitting indefinite
    # continuation into ambiguous calls.
    breaker = TransportBreaker(prov, F.INFRA_BREAKER_N)
    envs = orchestrate.load_envelopes(
        "/home/hatch/workspace/openline-airlock/study/econ-001/CONFIG.json")
    cx = {"ledger": led, "provider": breaker, "envs": envs, "stop": stop,
          "raw": raw, "run": RUNS, "breaker": breaker}
    ...
    except (worker.OperatorStop, ledger_mod.InsufficientBudget,
            InfraCircuitBreaker) as e:
