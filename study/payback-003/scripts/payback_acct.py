"""PAYBACK-003 cumulative accounting layer (offline).

Byte-equivalent logic to PAYBACK-002's accounting; the only changes are the
invocation-ID namespace (pb3-*), parameterized via F.INV_PREFIX, and one
apparatus repair (see below).

Apparatus repair (earned by PAYBACK-002, frozen before PAYBACK-003 contact):
the real ledger records unresolved reservations under the key "reservation",
not "retained". The reader now accepts either key. The frozen economic
...
def compute(ledger_path: str, tasks_path: str | None = None,
    ...
    for line in open(ledger_path):
        ...
        if k == "settle":
            ...
            # Apparatus repair (PAYBACK-003, earned by PAYBACK-002): the
            # frozen econ ledger records unresolved reservations under the
            # key "reservation"; this reader previously expected "retained"
            # and raised KeyError on any real ledger with unresolved entries.
            # Accept "retained" if present (defensive); the frozen economic
            # formulas below are unchanged.
            retained[e["invocation_id"]] = (
                e["retained"] if "retained" in e else e["reservation"])
