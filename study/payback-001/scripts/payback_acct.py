"""PAYBACK-001 cumulative accounting layer (offline).
...
def _op_index(inv: str):
    # pb1-o-C-<i> / pb1-o-T-<i>
    p = inv.split("-")
    if len(p) == 4 and p[0] == "pb1" and p[1] == "o" and p[2] in ("C", "T"):
        return p[2], int(p[3])
    return None, None
...
def compute(ledger_path: str, tasks_path: str | None = None,
    ...
    if run_dir and os.path.exists(run_dir + "/RUN_STATUS.json"):
        halted = json.load(open(run_dir + "/RUN_STATUS.json")).get("status") != "completed"
    if tasks_path and os.path.exists(tasks_path):
        rows = sum(1 for _ in open(tasks_path))
    both_settled = sum(1 for i in range(F.H)
                       if i in op_c and i in op_t)
    if halted or (rows is not None and rows < F.H) or \
