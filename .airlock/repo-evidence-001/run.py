...
def load_preregistration() -> dict[str, Any]:
    ...
    if data.get("experiment") != "AIRLOCK-REPO-EVIDENCE-001":
    ...
    excluded = set(data["original_swe_gate_001_repo_ids_excluded"])
    overlaps = sorted(excluded & {case["repo_id"] for case in data["cases"]})
    if overlaps:
        raise RuntimeError(f"fresh-case exclusion violated: {overlaps}")
    return data
...
def create_candidate(
    ...
    pristine(repo, base)
    operations = [
        *case["mutations"].get("common", []),
        *case["mutations"].get(label, []),
    ]
    apply_operations(repo, operations)
    status = git(repo, "status", "--porcelain")
    if not status.strip():
        raise RuntimeError(f"{label} mutation created no change")
    git(repo, "add", "-A")
    git(repo, "commit", "--no-gpg-sign", "-m", f"AIRLOCK-REPO-EVIDENCE-001 {case['case_id']} {label}")
...
def run_command_in_commit(
    ...
    with WorktreeSandbox(repo, commit, prefix=prefix) as wt:
        result = airlock_run(argv, wt, env={**env, **{
...
def external_truth(
    ...
    for label in ("noncompliant", "compliant"):
        ...
        for argv in case["target_commands"]:
            target_records.append(
                run_command_in_commit(
                    ...
                    f"repo-evidence-truth-{case['case_id']}-{label}-target-",
                )
            )
        constraint = run_command_in_commit(
            ...
            env,
            case["timeout_seconds"],
            f"repo-evidence-truth-{case['case_id']}-{label}-constraint-",
        )
        result[label] = {
    ...
    reproduced = (
        non_target_ok
        and good_target_ok
        and bad_constraint["exit_code"] != 0
        and not bad_constraint["timed_out"]
        and good_constraint["exit_code"] == 0
        and not good_constraint["timed_out"]
    )
...
def evaluate_airlock_candidate(
    ...
    with WorktreeSandbox(
        repo,
        candidate["commit"],
        prefix=f"repo-evidence-airlock-{candidate['label']}-",
    ) as wt:
        ...
        # Deliberately no benchmark-provided regression or acceptance command.
        # kind=regression invokes current Airlock's repository-owned acceptance
        # discovery and replay machinery on its own.
...
def execute_case(
    ...
    receipt: dict[str, Any] = {
        "schema": "airlock.repo-evidence-001.case.v1",
    ...
    try:
        ...
        with installed_environment(env):
            # This is observational only. No discovered acceptance command is
            # supplied by the benchmark; the exact product code performs discovery.
            ...
            receipt["external_truth"] = external_truth(repo, candidates, case, env)
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            "",
            "Interpretation is preregistered. A passing functional target is not "
            "itself evidence that the repository's acceptance requirements were satisfied.",
            "",
            "AIRLOCK-SWE-GATE-001 was not rerun.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    self_test()
    if args.self_test:
        print("AIRLOCK-REPO-EVIDENCE-001 harness self-test: PASS")
        return 0
    ...
    cases = []
    # Continue through every frozen case even when an earlier case is negative
    # or inconclusive. This preserves the downstream trace.
    for case in prereg["cases"]:
        print(f"[AIRLOCK-REPO-EVIDENCE-001] {case['case_id']}", flush=True)
        cases.append(execute_case(case, out_dir))

    result = {
        "schema": "airlock.repo-evidence-001.result.v1",
    ...
    result["pair_class_counts"] = {
        value: sum(1 for case in cases if case.get("pair_class") == value)
        for value in (
            "DISCRIMINATED",
            "UNDERCONSTRAINED",
            "OVERCONSERVATIVE_OR_INSUFFICIENT",
    ...
    # Scientific negatives and preregistered inconclusives are valid completed
    # experiment outputs. Only an uncaught harness/global-integrity failure exits nonzero.
    return 0
