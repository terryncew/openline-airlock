...
def load_preregistration() -> dict[str, Any]:
    ...
    if data.get("experiment") != "AIRLOCK-REPO-EVIDENCE-004":
...
def setup_case(repo: Path, case_root: Path, case: dict[str, Any], venv: Path) -> list[dict[str, Any]]:
    ...
    if tracked["exit_code"] != 0 or tracked["stdout_tail"].strip():
        raise RuntimeError("setup changed tracked repository state")
    return records
...
def create_candidate(repo: Path, base: str, case: dict[str, Any], label: str) -> dict[str, Any]:
    ...
    git(repo, "add", "-A")
    git(repo, "commit", "--no-gpg-sign", "-m", f"AIRLOCK-REPO-EVIDENCE-004 {case['case_id']} {label}")
...
def run_acceptance_suite(
    ...
    return [
        run_command_in_commit(repo, commit, argv, env, timeout, f"{prefix}-{index}-")
...
def external_truth(
    ...
    baseline = run_acceptance_suite(
        repo, base, commands, env, case["timeout_seconds"], f"repo-evidence-004-{case['case_id']}-base"
    )
    result: dict[str, Any] = {"base_acceptance_suite": baseline}
    for label in ("noncompliant", "compliant"):
        candidate = candidates[label]
        targets = [
            run_command_in_commit(
                repo, candidate["commit"], argv, env, case["timeout_seconds"],
                f"repo-evidence-004-{case['case_id']}-{label}-target-",
            )
            for argv in case["target_commands"]
        ]
        acceptance = run_acceptance_suite(
            repo, candidate["commit"], commands, env, case["timeout_seconds"],
            f"repo-evidence-004-{case['case_id']}-{label}-acceptance"
        )
        result[label] = {"target_commands": targets, "acceptance_suite": acceptance}

    base_green = all(r["exit_code"] == 0 and not r["timed_out"] for r in baseline)
    ...
    non_red = any(
        r["exit_code"] != 0 and not r["timed_out"]
        for r in result["noncompliant"]["acceptance_suite"]
    )
    good_green = all(
        r["exit_code"] == 0 and not r["timed_out"]
        for r in result["compliant"]["acceptance_suite"]
    )
    result["preconditions"] = {
        ...
        "noncompliant_acceptance_red": non_red,
        "compliant_acceptance_green": good_green,
    }
...
def evaluate_airlock_candidate(
    ...
    with WorktreeSandbox(repo, candidate["commit"], prefix=f"repo-evidence-004-airlock-{candidate['label']}-") as wt:
...
def aggregate_verdict(case: dict[str, Any]) -> str:
    ...
    if pair == "DISCRIMINATED":
        return "UVX_WRAPPER_CORRECTION_EARNED"
    if pair == "UNDERCONSTRAINED":
        return "CURRENT_UVX_WRAPPER_ACCEPTANCE_INCOMPLETE"
    if pair == "INVERTED":
...
def execute_case(case: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        "schema": "airlock.repo-evidence-004.case.v1",
        "case_id": case["case_id"],
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "The external acceptance suite is ground truth only. It was never supplied to Airlock.",
        "",
        "AIRLOCK-REPO-EVIDENCE-003 remains the frozen underconstraint receipt and was not rerun.",
    ]) + "\n"
...
def self_test() -> None:
    ...
    case = prereg["case"]
    assert case["repo_id"] == "emhass"
    assert case["external_acceptance_commands"][0][:3] == ["uvx", "ruff", "check"]
    assert case["external_acceptance_commands"][1][:3] == ["uvx", "ruff", "format"]
    ...
    for label in ("compliant", "noncompliant"):
        for op in case["mutations"][label]:
            ...
            assert op["path"] not in {"pyproject.toml", ".pre-commit-config.yaml", ".pre-commit-config.yml"}
...
def main() -> int:
    ...
    self_test()
    if args.self_test:
        print("AIRLOCK-REPO-EVIDENCE-004 harness self-test: PASS")
        return 0
    ...
    result = {
        "schema": "airlock.repo-evidence-004.result.v1",
