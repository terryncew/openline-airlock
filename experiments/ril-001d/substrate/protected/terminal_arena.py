...
CHANCE_IF_DECISIVE_EVIDENCE_MISSING = 0.25

# RIL-001D keeps the same six maintenance families but removes the RIL-001R escape hatch
# that let the live model browse every case file after policy-selected evidence was preloaded.
# Each case therefore measures a bounded search policy: the generator chooses receiver-defined
# evidence tactics, and the live model reasons over only the evidence those tactics authorize.
...
def _case(seed_rng: random.Random, family: str, index: int) -> Case:
    ...
    if family == "bug":
        ...
        decisive = ("read_tests", "trace_failure")
        ...
        files = {
            ...
            "baseline.txt": "baseline: outputs are correct but throughput is below the receiver threshold; the baseline alone does not localize the hot path.\n",
            ...
            "history.txt": f"history: {component} entered the hot path in the last refactor, but that is not itself a measurement.\n",
            "adversarial.txt": "adversarial: malformed input must still be rejected.\n",
        }
        decisive = ("measure_hotspot",)
        ...
        decisive = ("preserve_invariants", "read_docs", "compare_history")
    elif family == "flaky":
        ...
        files = {
            ...
            "baseline.txt": "baseline: repeated runs alternate pass/fail without code changes; the baseline does not reveal state ordering.\n",
        ...
        files = {
            ...
            "constraints.txt": "constraint: no flag day; mixed-version fleets are expected. New writers may advance, readers must remain backward-compatible during the window.\n",
        ...
        decisive = ("run_baseline", "read_tests")
    else:
        raise ValueError(f"unknown family: {family}")

    task = (
        f"Case {cid}. {context}\n"
        "Select exactly one option ID from the task record using only receiver-authorized evidence preloaded for this case."
    )
    return Case(cid, family, task, options, answer, files, decisive)
...
def evidence_sufficient(case: Case, tactics: list[str]) -> bool:
    chosen = set(tactics)
    return any(tactic in chosen for tactic in case.decisive_tactics)


def calibration_projection(cases: list[Case], tactics_by_case: dict[str, list[str]]) -> dict[str, Any]:
    sufficient = 0
    by_family: dict[str, dict[str, int]] = {}
    for case in cases:
        tactics = tactics_by_case.get(case.case_id, [])
        ok = evidence_sufficient(case, tactics)
        sufficient += int(ok)
        bucket = by_family.setdefault(case.family, {"sufficient": 0, "total": 0})
        bucket["total"] += 1
        bucket["sufficient"] += int(ok)
    total = len(cases)
    coverage = sufficient / total if total else 0.0
    projected = coverage + (1.0 - coverage) * CHANCE_IF_DECISIVE_EVIDENCE_MISSING
    return {
        "cases": total,
        "decisive_evidence_sufficient": sufficient,
        "decisive_evidence_coverage": coverage,
        "chance_if_insufficient": CHANCE_IF_DECISIVE_EVIDENCE_MISSING,
...
def materialize_visible(root: str | Path, cases: list[Case]) -> dict[str, Any]:
    ...
    value = {"schema": "openline.ril-001d.terminal-visible.v1", "cases": manifest}
