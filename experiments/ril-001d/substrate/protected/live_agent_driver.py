#!/usr/bin/env python3
"""Pinned-Hermes live driver with explicit interaction/output-token limits.

Runs inside the pinned Hermes virtualenv. Unlike the public ``hermes -z`` wrapper at the
RIL-001 pin, this constructs AIAgent directly so max_iterations and max_tokens are receiver-set
constructor arguments rather than prose/config assumptions. The driver does not decide whether
an output survives; it only runs the live worker and records provider/model usage telemetry.
"""
...
def main() -> int:
    ...
    # Imports happen only in the live runtime, inside the exact pinned Hermes venv.
    ...
    try:
        agent = AIAgent(
            ...
            fallback_model=None,  # fail rather than silently switching the matched model
    ...
    except BaseException as exc:  # preserve usage fields already accumulated when possible
    ...
    usage.update({
        "schema": "openline.ril-001.hermes-usage.v1",
