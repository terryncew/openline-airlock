...
def main() -> int:
    ...
    for task_path in sorted((ROOT / "cases").glob("*/TASK.json")):
        ...
        if "inspect_repo" in tactics:
            names = sorted(p.name for p in cdir.iterdir() if p.is_file() and p.name != "TASK.json")
            evidence.append("visible evidence filenames: " + ", ".join(names))
            evidence_names.append("<filenames-only>")
        ...
        evidence_receipts[case_id] = list(dict.fromkeys(evidence_names))
        case_sections.append(
            json.dumps(task, sort_keys=True)
            + "\nReceiver-authorized policy-selected evidence:\n"
            + ("\n".join(dict.fromkeys(evidence)) if evidence else "(no evidence content selected)")
        )

    # The RIL-001R arena saturated because the live model could recover by browsing every
    # case directory after weak preloading. RIL-001D makes the search-policy treatment real:
    # once the receiver has constructed the prompt, unselected case evidence is removed before
    # model contact. The same rule applies to both arms.
    cases_root = ROOT / "cases"
    shutil.rmtree(cases_root)
    case_files_removed_before_model_contact = not cases_root.exists()

    prompt = (
        "RIL-001D terminal bounded-evidence maintenance search. Solve every case using the task options and only the "
        "receiver-authorized evidence preloaded below. Unselected case files are removed before model contact, so do not "
        "assume additional case evidence can be browsed. Return ONLY one JSON object mapping every case_id to exactly one "
        "option ID A/B/C/D.\n\n"
    ...
    receipt = {
        "schema": "openline.ril-001d.terminal-worker.v3",
        ...
        "preloaded_evidence_files": evidence_receipts,
        "case_files_removed_before_model_contact": case_files_removed_before_model_contact,
