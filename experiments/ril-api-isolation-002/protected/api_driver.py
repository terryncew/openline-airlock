...
Usage:
    ...
    python3 protected/api_driver.py qualify --output /tmp/ril-api-isolation-002
...
RECEIPT_SCHEMA = "openline.ril-api-isolation-002.receipt.v1"
HTTP_TIMEOUT = 60
...
def cmd_self_check() -> int:
    ...
    assert core.aggregate(good) == "PASS_RIL_API_ISOLATION_002_STATELESS_CANARY_BOUNDARY"
    leak = [
...
def cmd_qualify(output: Path) -> int:
    ...
    out_path = output / "RIL_API_ISOLATION_002_RECEIPT.json"
    out_path.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n")
    print(f"verdict={verdict}")
    print(f"receipt={out_path}")
    return 0 if verdict == "PASS_RIL_API_ISOLATION_002_STATELESS_CANARY_BOUNDARY" else 2
