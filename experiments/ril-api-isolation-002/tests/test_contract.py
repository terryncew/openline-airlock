...
def test_payload_allowlist():
    ...
    assert "temperature" not in p and p["max_completion_tokens"]==100
...
def test_clean_pair_passes():
    ...
    assert m.aggregate(pairs)=="PASS_RIL_API_ISOLATION_002_STATELESS_CANARY_BOUNDARY"
...
def test_prereg_frozen_contract():
    p=json.loads((ROOT/"RIL_API_ISOLATION_002_PREREGISTRATION.json").read_text())
    assert p["experiment"]=="RIL-API-ISOLATION-002"
    assert p["substrate"]["model_id"]=="gpt-6-astra"
    assert "temperature" not in p["protocol"]
    assert p["protocol"]["max_completion_tokens"]==100
