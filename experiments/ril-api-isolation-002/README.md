# RIL-API-ISOLATION-002 — API-request statelessness qualification for canary isolation
...
Four Chat Completions calls plus one `GET /v1/models`, no sampling parameters
pinned, max_completion_tokens 100. Expected spend is a fraction of a cent; the frozen ceiling is
$0.50.

## Lineage note

RIL-API-ISOLATION-001 froze terminal on 2026-09-14 with verdict
`PROTOCOL_FAILURE_UNCLASSIFIED`: its protocol pinned `temperature: 0` and
`max_tokens`, and gpt-6-astra rejects both (HTTP 400 — only the default
temperature is supported; `max_tokens` must be `max_completion_tokens`). It
failed closed before any trial call completed. This -002 package is the
corrected re-run under a new ID, per the anti-rescue clause: identical
protocol except no temperature is sent and the allowlist carries
