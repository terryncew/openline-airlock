...
def select(rows: list[dict], minimum_gap: float) -> dict:
    ...
    ordered = sorted(
        eligible,
        key=lambda r: (float(r["net_gain_score"]),
...
def materialize(repo: Path, patch: str, cid: str):
    wt = Path(tempfile.mkdtemp(prefix="search001b-eval-"))
    wt.rmdir()
    ...
    if cp.returncode:
        raise RuntimeError(cp.stderr.strip())
    try:
        subprocess.run(["git", "config", "user.name", "SEARCH-001B Evaluator"], cwd=wt, check=True)
        ...
        subprocess.run(["git", "commit", "-m", f"SEARCH-001B candidate {cid}"], cwd=wt,
...
def main() -> int:
    ...
    if not args.bundle or not args.out:
        ap.error("--bundle and --out required unless --self-check")

    bundle = json.loads(Path(args.bundle).read_text())
    if (
        bundle.get("schema") != "airlock.search-001b.candidate-bundle.v1"
        or bundle.get("substrate_commit") != SUBSTRATE
        or bundle.get("strategy") != "evidence_first"
        or bundle.get("model") != "gpt-5.6-sol"
        or bundle.get("requested_candidates") != 4
        or not bundle.get("public_preflight_green")
    ):
        raise RuntimeError("SEARCH-001B bundle boundary mismatch")

    tournament_status = (bundle.get("tournament") or {}).get("status")
    if tournament_status in {"BASELINE_NOT_GREEN", "INFRA_FAILURE", "ERROR"}:
        output = {
            "schema": "airlock.search-001b.result.v1",
            "outcome": "INVALID_EXPERIMENT",
            "reason": tournament_status,
        }
    else:
        ...
        for candidate in bundle.get("candidates", []):
            ...
            if (candidate.get("search_filter") or {}).get("status") != "PASS":
                rows.append({
                    "candidate_id": cid,
                    "disposition": "REJECT",
                    "reason": (candidate.get("search_filter") or {}).get("reason") or "SEARCH_FILTER_FAILED",
                })
                continue

            patch = candidate.get("patch") or ""
            if not patch:
                rows.append({"candidate_id": cid, "disposition": "REJECT", "reason": "NO_PATCH"})
                continue
            ...
            try:
                ...
                row["probe"] = {
                    "baseline": candidate.get("baseline_probe"),
                    "candidate": candidate.get("candidate_probe"),
                    "delta": candidate.get("measured_delta"),
                    "dossier_sha256": candidate.get("probe_dossier_sha256"),
                }
                rows.append(row)
        ...
        output = {
            ...
            "outcome": "SEARCH_STRATEGY_GAIN" if won else "DISCOVERY_SEARCH_DEFICIT",
            "earned_label": "SEARCH_STRATEGY_GAIN" if won else None,
            "selection": selection,
            "candidates": rows,
            "claim": (
                "A measure-first autonomous search strategy found an independently admissible improvement under the unchanged frozen SELF-001 evaluator."
                if won else None
            ),
            "claim_boundary": (
                "The worker saw public repository evidence and its own locally reproducible probes only. "
                "It had no access to the hidden evaluator, hidden target registry, prior hidden feedback, "
                "or the positive-control patch. SEARCH-002 fresh-target replication remains required before compounding."
            ),
    ...
    print(json.dumps({"outcome": output["outcome"]}))
    return 0
