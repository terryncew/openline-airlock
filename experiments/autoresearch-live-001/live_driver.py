...
def verify_protected(repo: Path, expected: dict[str, str]) -> dict[str, Any]:
    status = clean_status(repo)
    dirty_lines = [line for line in status.splitlines() if line and not line.endswith(" train.py") and not line.endswith("train.py")]
    # Evaluation requires a fully committed candidate. Any uncommitted path,
    # including train.py, is a protocol error; keep separate detail for evidence.
    if status:
...
def muse_version() -> str | None:
    p = run(["muse", "--version"], check=False)
    if p.returncode:
        return None
    return (p.stdout.strip() or p.stderr.strip() or None)
...
def prereg_path() -> Path:
    return Path(__file__).with_name("AUTORESEARCH_LIVE_001_PREREGISTRATION.json").resolve()
...
def command_snapshot(repo: Path, evidence: Path) -> None:
    ...
    print(json.dumps({"status": "SNAPSHOT_FROZEN", "bootstrap_commit": head, "protected_files": len(protected), "hardware": state["hardware"]}, sort_keys=True))
...
def finalize_result(repo: Path, evidence: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "muse_version": state.get("muse_version"),
        ...
        "claim_boundary": "receiver-owned fixed-seed governance observation; not statistical ML superiority or hostile-process isolation",
    }
    result_path = evidence / "AUTORESEARCH_LIVE_001_RESULT.json"
...
def command_step(repo: Path, evidence: Path, cache: Path, candidate_arg: str) -> None:
    ...
    if state.get("muse_version") is None:
        state["muse_version"] = muse_version()
        save_state(evidence, state)
    ...
    if decision == "ACCEPT":
        ...
        # Preserve rejected proposal locally, then restore the research branch to
        # receiver-accepted state so proposal 2 cannot inherit rejected work.
    ...
    if state["terminal"]:
        ...
        out["result"] = str(evidence / "AUTORESEARCH_LIVE_001_RESULT.json")
...
def command_finalize(repo: Path, evidence: Path) -> None:
    ...
    result = finalize_result(repo, evidence, state)
    print(json.dumps({"result": str(evidence / "AUTORESEARCH_LIVE_001_RESULT.json"), "terminal_verdict": result["terminal_verdict"]}, sort_keys=True))
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="External live driver for AUTORESEARCH-LIVE-001")
    p.add_argument("--repo", default="/workspace/autoresearch-live-001")
    p.add_argument("--evidence", default="/workspace/autoresearch-live-evidence")
