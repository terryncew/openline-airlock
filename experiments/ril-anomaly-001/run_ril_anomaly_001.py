...
EXPERIMENT = "RIL-ANOMALY-001"
SCHEMA = "openline.ril-anomaly-001.result.v1"
PREPARED_SCHEMA = "openline.ril-anomaly-001.prepared.v1"
STATE_SCHEMA = "openline.ril-anomaly-001.receiver-state.v1"
PACKET_SCHEMA = "openline.ril-anomaly-001.research-packet.v1"
OUTCOME_SCHEMA = "openline.ril-anomaly-001.receiver-outcome.v1"

FROZEN_MAIN = "3c610ba2840c7a5e4d90d677940b2898977bfb78"
...
RESEARCHER = {"product": "Muse", "model": "Muse Spark 1.3", "name": "Prince"}
ALLOWED_TOOLS = [
    "Muse conversational reasoning",
    "Muse Linux VM shell/filesystem",
    "Muse browser/internet",
    "RunPod GPU",
]
...
ROOT = HERE.parents[1]
PREREG_PATH = HERE / "RIL_ANOMALY_001_PREREGISTRATION.json"
DRIVER_PATH = HERE / "protected" / "research_driver.py"
ANOMALY_PROTOCOL_PATH = HERE / "PRINCE_PROTOCOL.md"
...
def verify_repo(root: Path = ROOT) -> dict[str, Any]:
    ...
    if not seed_path.is_file():
        raise RuntimeError(f"seed freeze missing: {SEED_FREEZE_PATH}")
    ...
    if seed.get("recursive_improvement_standing") != "NOT_EARNED":
        raise RuntimeError("seed recursive boundary changed")
    ...
    if git_available(root):
        ...
        blob = sh("git", "rev-parse", f"HEAD:{SEED_FREEZE_PATH.as_posix()}", cwd=root)
        if blob != SEED_FREEZE_BLOB_SHA1:
            raise RuntimeError(f"seed freeze blob changed: {blob}")
    return {
        "verified": True,
        "frozen_main": FROZEN_MAIN,
        "head": head,
        "seed_freeze_blob_sha1": blob or SEED_FREEZE_BLOB_SHA1,
        "seed_freeze_sha256": sha256_file(seed_path),
...
def seed_anomaly(seed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "observed_failure": (
            "Two fresh Prince/Muse Spark 1.3 sessions received matched packets whose only experimental difference "
        ...
        "boundary": (
            "This observation applies to Prince under the frozen LIVE-002 protocol. It does not establish that every "
...
def initial_head(seed_sha256: str) -> str:
    return sha256_bytes(canonical({"experiment": EXPERIMENT, "seed_freeze_sha256": seed_sha256, "accepted_lessons": []}))
...
def packet_for(state: dict[str, Any], session_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "anti_rescue": (
            ...
            "promotion rule, researcher substrate, or scoring threshold under this experiment ID."
        ),
    }
...
def score(prepared: Path, output: Path) -> dict[str, Any]:
    ...
    if any(metrics[m]["terminal"]["status"] in {"PROTOCOL_FAILURE", "RECEIVER_OUTCOME_INVALID", "BUDGET_EXHAUSTED"} for m in MODES):
        verdict = "INCONCLUSIVE_RIL_ANOMALY_001_TERMINAL_FAILURE"
    elif advantage >= REQUIRED_PROMOTION_ADVANTAGE:
        verdict = "PASS_RIL_ANOMALY_001_EARNED_RESEARCH_RATCHET"
    elif advantage <= -REQUIRED_PROMOTION_ADVANTAGE:
        verdict = "OBSERVED_RIL_ANOMALY_001_ORDINARY_SEARCH_ADVANTAGE"
    else:
        verdict = "NO_OBSERVED_RIL_ANOMALY_001_ADVANTAGE"
    result = {
        "schema": SCHEMA,
        "experiment": EXPERIMENT,
...
def self_check() -> dict[str, Any]:
    ...
    fake_response = {
        ...
        "tools_used": ["Muse conversational reasoning"],
    }
    packet = {
        "experiment": EXPERIMENT,
        "session_id": "session-1",
