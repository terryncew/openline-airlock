"""Contract tests for RSI-006-Q substrate qualification.
...
mutation, no researcher/model-access code path, and the frozen budgets.
They do NOT qualify the substrate and involve no arm performance.
"""
...
def test_operator_set_matches_spec():
    ...
    spec = (EXP_DIR / "RSI_006_Q_SPEC.md").read_text()
...
def test_no_scientific_primary_surface():
    src = (EXP_DIR / "run_rsi_006_q.py").read_text()
    tree = ast.parse(src)
    for node in ast.walk(tree):
