# RIL-API-ISOLATION-001 operator card
...
```
python3 experiments/ril-api-isolation-001/protected/api_driver.py self-check
```
...
```
python3 experiments/ril-api-isolation-001/protected/api_driver.py qualify \
  --output /tmp/ril-api-isolation-001
```
...
`RIL_API_ISOLATION_001_RECEIPT.json` (schema
`openline.ril-api-isolation-001.receipt.v1`). Report the verdict verbatim.

## Hard rules

- Chat Completions only. Never Responses, never Assistants, never threads.
- Payload keys are limited to model, messages, temperature, max_tokens.
  Anything else fails closed before sending.
...
- After the first primary API call, the anti-rescue clause freezes the pinned
  model id, prompts, temperature, trial count, pass rule, payload allowlist,
